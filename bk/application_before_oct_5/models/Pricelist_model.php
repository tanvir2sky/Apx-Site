...
public function getPriceListAllData($where=NULL,$order=NULL,$type=NULL,$start=NULL,$end=NULL) {
    $this->db->select('pl.*,(select count(zone_id) from apx_zone where list_id=pl.list_id) as total_zone,adm1.user_name as created_name,adm2.user_name as modified_name');
    $this->db->from('apx_price_list pl');
        if($start >=0 && $end > 0){
