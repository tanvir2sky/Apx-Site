...
Class Zone_model_f extends CI_Model
{
    /*
    * GET ALL Zone RECORD QUERY
    */
    public function getZoneAllData($where=NULL,$order=NULL,$type=NULL,$start=NULL,$end=NULL) {
        $this->db->select('zn.*,pl.listName,pl.listCode,(select count(zc_id) from apx_zone_countries_f where zone_id=zn.zone_id) as total_countries,(select count(wp_id) from apx_weight_prices_f where zone_id=zn.zone_id) as total_wp,adm1.user_name as created_name,adm2.user_name as modified_name');
        $this->db->from('apx_zone_f zn');
        if($start >=0 && $end > 0){
        ...
        $this->db->join('apx_price_list_f pl', 'pl.list_id = zn.list_id');
    ...
    */
    public function getZoneCountriesData($where=NULL,$order=NULL,$type=NULL,$start=NULL,$end=NULL) {
        ...
        $this->db->join('apx_zone_countries_f zc', 'crt.countryCode = zc.countryCode','LEFT');
